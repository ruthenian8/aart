...
class MultiTaskPipeline(GenericPipeline):
    ...
    def add_predictions(self, df, preds):
        ...
        pred_cols = {}
        if self.params.approach == "single":
            df["majority_pred"] = pd.Series(
                np.argmax(preds.predictions["majority_label"], axis=-1)
            )
        else:
            for i, true_label_col in enumerate(annotators):
                ...
                pred_cols[f"pred_{pred_label_col}"] = pd.Series(
                    np.argmax(preds.predictions[pred_label_col], axis=-1)
                )
            ...
            df["majority_pred"] = self.calculate_majority(
                pred_cols, pred_cols.columns.tolist()
            )
